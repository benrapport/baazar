...
from bazaar.types import (
    ExchangeResult, CallRequest, LLMConfig, ExchangeConfig, JudgeConfig,
    SubmissionPayload,
)
...
class TestExchangeClient:
    ...
    def test_call_negative_max_price_raises(self):
        ...
        with pytest.raises(ValueError):
            ex.call(llm={"input": "test"},
                    exchange={"max_price": -1.0})

    def test_call_zero_max_price_raises(self):
        ex = Exchange(api_key="demo")
        with pytest.raises(ValueError):
            ex.call(llm={"input": "test"},
                    exchange={"max_price": 0.0})

    def test_call_negative_timeout_raises(self):
        ex = Exchange(api_key="demo")
        with pytest.raises(ValueError):
            ex.call(llm={"input": "test"},
                    exchange={"max_price": 5.0, "timeout": -1.0})

    def test_call_zero_timeout_raises(self):
        ex = Exchange(api_key="demo")
        with pytest.raises(ValueError):
            ex.call(llm={"input": "test"},
                    exchange={"max_price": 5.0, "timeout": 0.0})
    ...
    def test_server_not_running_raises_connection_error(self):
        ...
        with pytest.raises(ConnectionError, match="Cannot connect"):
            ex.call(llm={"input": "test"},
                    exchange={"max_price": 5.0, "timeout": 2.0})

    def test_call_json_schema_without_schema_raises(self):
        ex = Exchange(api_key="demo")
        with pytest.raises(ValueError, match="json_schema"):
            ex.call(llm={"input": "test",
                         "response_format": {"type": "json_schema"}},
                    exchange={"max_price": 5.0})

    def test_call_invalid_response_format_type_raises(self):
        ex = Exchange(api_key="demo")
    ...
    def test_min_quality_out_of_range_rejected(self):
        ...
        with pytest.raises(ValidationError):
            JudgeConfig(min_quality=0)
        with pytest.raises(ValidationError):
            JudgeConfig(min_quality=11)
